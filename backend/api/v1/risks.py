...
def create_risk(
    ...
    """在项目下创建风险"""
    PermissionChecker.require_risk_permission(current_user, action="modify")
...
def get_risks(
    ...
    return RiskService.get_list(
        ...
        status=status_filter, owner_name=owner_name,
    )
...
def update_risk(
    ...
    # 权限检查（基于角色）
    PermissionChecker.require_risk_permission(current_user, risk, action="modify")
...
def delete_risk(
    ...
    # 权限检查（基于角色）
    PermissionChecker.require_risk_permission(current_user, risk, action="delete")

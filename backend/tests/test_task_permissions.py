"""任务权限测试（基于角色）：ADMIN/PROJECT_MANAGER 可改，MEMBER/OBSERVER 只读。"""
import pytest
...
def _make_project_with_task(db):
    project = Project(name="权限项目", record_date=date(2026, 5, 30), owner_name="负责人")
    db.add(project)
    db.commit()
    db.refresh(project)
    task = Task(project_id=project.id, name="权限任务", owner_name="负责人")
    db.add(task)
    db.commit()
    db.refresh(task)
    return project, task
...
def test_manager_roles_can_update_task(db_session, role):
    user = _make_user(db_session, f"mgr_{role.value}", role=role)
    _, task = _make_project_with_task(db_session)
    client = _client(db_session, user)
    try:
        r = client.put(f"/api/v1/tasks/{task.id}", json={"name": "改名"})
...
def test_non_manager_roles_cannot_update_task(db_session, role):
    user = _make_user(db_session, f"u_{role.value}", role=role)
    _, task = _make_project_with_task(db_session)
    client = _client(db_session, user)
    try:
        r = client.put(f"/api/v1/tasks/{task.id}", json={"name": "越权"})
        assert r.status_code == 403
    finally:
        _cleanup()


def test_member_cannot_delete_task(db_session):
...
def test_admin_can_delete_task(db_session):
...
def test_member_cannot_create_task(db_session):
    member = _make_user(db_session, "member", role=UserRole.MEMBER)
    project, _ = _make_project_with_task(db_session)
    client = _client(db_session, member)
    try:
        r = client.post(f"/api/v1/projects/{project.id}/tasks",
                        json={"name": "新任务"})
        assert r.status_code == 403
    finally:
        _cleanup()
...
def test_update_nonexistent_task_404(db_session):
    admin = _make_user(db_session, "admin", role=UserRole.ADMIN)
    client = _client(db_session, admin)
    try:
        r = client.put("/api/v1/tasks/99999", json={"name": "x"})

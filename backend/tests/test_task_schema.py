...
def test_task_create_minimal_valid():
    ...
    task = TaskCreate(name="任务A", owner_name="负责人")
    assert task.name == "任务A"
    assert task.owner_name == "负责人"
...
def test_task_name_required():
    ...
    with pytest.raises(ValidationError):
        TaskCreate(owner_name="负责人")


def test_task_name_empty_invalid():
    """name 为空字符串应校验失败"""
    with pytest.raises(ValidationError):
        TaskCreate(name="", owner_name="负责人")


def test_task_owner_name_optional():
    """owner_name 可选：不提供也能创建（与用户账号解耦）"""
    task = TaskCreate(name="任务")
...
def test_task_completion_out_of_range():
    ...
    with pytest.raises(ValidationError):
        TaskCreate(name="任务", completion=101)
    with pytest.raises(ValidationError):
        TaskCreate(name="任务", completion=-1)


def test_task_parent_id_must_be_positive():
    """parent_task_id 若提供必须为正数"""
    with pytest.raises(ValidationError):
        TaskCreate(name="任务", parent_task_id=0)
...
def test_task_update_partial():
    """TaskUpdate 部分字段更新"""
    update = TaskUpdate(completion=80, status=TaskStatus.COMPLETED, owner_name="新人")
    data = update.model_dump(exclude_unset=True)
    assert data == {"completion": 80, "status": TaskStatus.COMPLETED, "owner_name": "新人"}
